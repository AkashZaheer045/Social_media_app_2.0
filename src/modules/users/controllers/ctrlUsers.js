...
const update = async function (req, res, next) {
  try {
    ...
    const [updatedUser, err2] = await userInstance.update(req.body);
    if (err2) {
      return next(err2);
    }
    // update preferences
...
const login = async function (req, res, next) {
  try {
    ...
    const access_token = authorization.accessToken;
    //-Assigning the userid and device token to the refreshtoken here from db-//
    let refresh_token = authorization.refreshToken;

    const encoded_user_id = base_encoder.encode(user.id, constants.data_set);
    const encoded_access_id = base_encoder.encode(authorization.id, constants.data_set);
    const now = Date.now();
    const accessExpiry = base_encoder.encode(now + 1000 * 60 * 86400, constants.data_set); // 1 day
    const refreshExpiry = base_encoder.encode(now + 1000 * 86400 * 3, constants.data_set); // 3 days

    const access_authorization =
      encoded_user_id +
      index_separator +
      encoded_access_id +
      index_separator +
...
const refreshToken = async (req, res, next) => {
  try {
    ...
    const authInstance = new sequelize.db(sequelize.models.authorizations);
...
const getUserTotalCount = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: 'Failed to fetch total user count', details: err.message });
  }
};
