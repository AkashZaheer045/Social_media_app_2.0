...
exports.toggleComment = async (req, res, next) => {
    try {
        ...
        const userId = req.user.userId;
        const [liked, err] = await likeService.toggleLike({ userId, targetId: commentId, targetType: 'comment' });
        if (err) return next(err);
        return next({ liked });
    } catch (error) {
        return next(error);
...
exports.getByPost = async (req, res, next) => {
    try {
        ...
        const [likes, err] = await likeService.getLikesByTarget({ targetId: postId, targetType: 'post' });
        if (err) return next(err);
        return next({ likes });
    } catch (error) {
        return next(error);
...
exports.getByComment = async (req, res, next) => {
    try {
        ...
        const [likes, err] = await likeService.getLikesByTarget({ targetId: commentId, targetType: 'comment' });
        if (err) return next(err);
        return next({ likes });
    } catch (error) {
        return next(error);
...
exports.getByUserAndPost = async (req, res, next) => {
    try {
        ...
        const [liked, err] = await likeService.checkUserLike({ userId, targetId: postId, targetType: 'post' });
        if (err) return next(err);
        return next({ liked });
    } catch (error) {
        return next(error);
...
exports.getByUserAndComment = async (req, res, next) => {
    try {
        ...
        const [liked, err] = await likeService.checkUserLike({ userId, targetId: commentId, targetType: 'comment' });
        if (err) return next(err);
        return next({ liked });
    } catch (error) {
        return next(error);
