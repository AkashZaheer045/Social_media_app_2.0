...
const findOrCreateResetToken = async (userId) => {
  ...
  const [existing, error] = await tempTokenInstance.findOne({ where: { userId, used: false } });
...
const markTokenUsed = async (tempTokenId) => {
  await sequelize.models.temp_tokens.update({ used: true }, { where: { id: tempTokenId } });
};
...
const getUserCount = async () => {
  ...
  return sequelize.models.users.count({ paranoid: false });
};
