...
ValidationRules.rule = (method) => {
  switch (method) {
    ...
    case 'toggleComment': {
      return [check('commentId').notEmpty().withMessage('Comment ID is required').isNumeric()];
    }
    case 'getByPost': {
      return [check('postId').notEmpty().withMessage('Post ID is required').isNumeric()];
    }
    case 'getByComment': {
      return [check('commentId').notEmpty().withMessage('Comment ID is required').isNumeric()];
    }
    case 'getByUser': {
      return [check('userId').notEmpty().withMessage('User ID is required').isNumeric()];
    }
